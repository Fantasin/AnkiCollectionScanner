...
def test_execute_sync_collection_success(mock_repo, mock_anki_client):
    usecase = SyncCollectionUseCase(mock_repo, mock_anki_client)
...
def test_execute_sync_collection_use_case_sync_error_repo(mock_repo, mock_anki_client):

    mock_repo.save_snapshot.side_effect = Exception("Test exception raised")
...
def test_execute_sync_collection_use_case_sync_error_anki_client(mock_repo, mock_anki_client):

    mock_anki_client.get_decks_and_ids.side_effect = Exception("Test exception raise")
...
def test_add_model_field_names(mock_repo, mock_anki_client, snapshot):
    usecase = SyncCollectionUseCase(mock_repo, mock_anki_client)
    ...
    mock_anki_client.get_model_field_names.side_effect = [
        ["Front", "Back"],
        ["Text"]
    ]
    ...
    assert mock_anki_client.get_model_field_names.call_count == 2
    snapshot.update_model_fields.assert_any_call(1, ["Front", "Back"])
    snapshot.update_model_fields.assert_any_call(2, ["Text"])

def test_add_note_id_field_data(mock_repo, mock_anki_client, snapshot):
    ...
    mock_anki_client.get_note_id_field_data.return_value = notes_data

    snapshot.update_note_data = Mock()

    usecase.add_note_id_field_data(snapshot)

    mock_anki_client.get_note_id_field_data.assert_called_once_with([10, 20])
    assert snapshot.update_note_data.call_count == 2

def test_add_deck_note_count_and_hash(mock_repo, mock_anki_client, snapshot):
    usecase = SyncCollectionUseCase(mock_repo, mock_anki_client)
    ...
    mock_anki_client.get_deck_note_ids.side_effect = [[112, 221, 312], [212,55]]
    ...
    assert mock_anki_client.get_deck_note_ids.call_count == 2
...
def test_sync_user_collection_happy_path(mock_repo, mock_anki_client):
    snapshot = Mock()

    use_case = SyncCollectionUseCase(mock_repo, mock_anki_client)

    # Mock base data fetch
    mock_anki_client.get_decks_and_ids.return_value = {"Default": 1}
    mock_anki_client.get_models_and_ids.return_value = {"Basic": 2}
    mock_anki_client.get_all_note_ids.return_value = [101, 102]
    ...
    # Verify fetch stage
    mock_anki_client.get_decks_and_ids.assert_called_once()
    mock_anki_client.get_models_and_ids.assert_called_once()
    mock_anki_client.get_all_note_ids.assert_called_once()
...
def test_sync_stops_if_fetch_fails(mock_repo, mock_anki_client):
    snapshot = Mock()

    use_case = SyncCollectionUseCase(mock_repo, mock_anki_client)

    mock_anki_client.get_decks_and_ids.side_effect = RuntimeError("fetch failed")

    with pytest.raises(RuntimeError):
        use_case.sync_user_collection(snapshot)

    # Should stop immediately
    mock_anki_client.get_models_and_ids.assert_not_called()
    mock_anki_client.get_all_note_ids.assert_not_called()
    snapshot.update_decks.assert_not_called()


def test_sync_stops_if_update_fails(mock_repo, mock_anki_client):
    snapshot = Mock()
    ...
    mock_anki_client.get_decks_and_ids.return_value = {"Default": 1}
    mock_anki_client.get_models_and_ids.return_value = {"Basic": 2}
    mock_anki_client.get_all_note_ids.return_value = [101]
...
def test_sync_stops_if_enrichment_fails(mock_repo, mock_anki_client):
    snapshot = Mock()
